...
def run_som_analysis(symbols, plot=True):
    ...
    # Use all historical data for all symbols
    ...
    # Assign clusters for each row (historical)
    ...
    # Analyze clusters (example: mean of selected features per cluster)
    cluster_centers = {}
...
symbols = get_symbols(get_dates_from_most_active_files()[-1], top_n=17)[0]
cluster_labels, cluster_centers = run_som_analysis(symbols, plot=True)
